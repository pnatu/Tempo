    ...
    public function uploadAvatar($request, $response) {
        ...
        if ($_FILES['avatar']['error'] === 0) {
            ...
            if (isset($imgsize['Content-Length'])) {
                $sizeImage = (float) ($imgsize['Content-Length']);
                if ($sizeImage < 1) {
                    $result123['success'] = 0;
...
//                var_dump($user);exit;
                                ...
                                if (count($devInfo) > 0) {
                                    foreach ($devInfo as $dev123) {
                                        if (isset($dev123->one_signal_userid)) {
                                            ...
                                            $this->sendOneSignalNotify($one_signal_userid, $Notifydata, $contentmsg, $network_user_id, 'setUserPosts');
                                        }
                                    }
                                }
                            }
                        ...
                        if ($pos === false) {
                            ...
                            if (count($devInfo) > 0) {
                                foreach ($devInfo as $dev123) {
                                    if (isset($dev123->one_signal_userid)) {
                                        $one_signal_userid = trim($dev123->one_signal_userid);
                                        $this->sendOneSignalNotify($one_signal_userid, $Notifydata, $contentmsg, $postData['postedtouserid'], 'setUserPosts');
                                    }
                                }
                            }
                        } else {
    ...
    public function setUserPosts09Feb($request, $response) {
        $postData = $request->getParsedBody();
        $basePath = $request->getUri()->getBasePath();
        $public_path = $_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . "aws_config" . DIRECTORY_SEPARATOR;
        include($public_path . 'config_s3.php');
...
//                        'isdeleted' => 1]);
    ...
    function sendOneSignalNotify($onesingaluseridarr, $data, $contentmsg, $userid = 0, $fromFunction = null) {
    ...
    function sendOneSignalNotify09Feb($onesingaluseridarr, $data, $contentmsg, $userid = 0) {
        $user_IsNotification = 1;
        if ($userid != 0) {
            $user123 = $this->_db->table('users')
                    ->select('IsNotification')
    ...
    function UploadVideoThumbnail($request, $response) {
        ...
        if (isset($_FILES['videothumbnail']) && $_FILES['videothumbnail']['error'] === 0) {
            ...
            $imgsize = get_headers($uploadedThumbimg, true);
            if (isset($imgsize['Content-Length'])) {
                $sizeImage = (float) ($imgsize['Content-Length']);
                if ($sizeImage < 1) {
                    $result123['success'] = 0;
